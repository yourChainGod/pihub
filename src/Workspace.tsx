...
export default function Workspace({ deviceId }: { deviceId: string }) {
  ...
  // Last reading position per session (null/absent = stay at the latest
  // messages); scrolling near the bottom is treated as "following live output"
  // and never recorded, so returning lists land back at the bottom.
  ...
  // Every switch lands on the remembered reading position when the user left
  // the session scrolled into history; otherwise (first open / bottom-follow)
  // stay at the latest messages. The detail state still holds the previous
  // session for one commit after a switch — wait for the frame where the
  // committed detail is the one detailRef just adopted, otherwise the pending
  // restore would be consumed against the old session.
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      ...
      if (!el) return;
      if (pending.top !== null) {
        // Restoring a history position must also stop bottom-follow, otherwise
        // the next streamed message immediately yanks the view back down.
  ...
  return <div className={`workspace-shell ${leftOpen ? "" : "left-closed"} ${rightOpen ? "" : "right-closed"} ${fullscreen ? "native-fullscreen" : ""}`} style={{ "--left-w": `${leftWidth}px`, "--right-w": `${rightWidth}px` } as React.CSSProperties}>
    ...
    <main className="conversation-pane">
      ...
      <div className="message-area">
        <div className="message-scroll" ref={messageScrollRef} onScroll={(event) => { const el = event.currentTarget; if (selectedIdRef.current && el.scrollHeight - el.scrollTop - el.clientHeight >= 140) scrollPositionsRef.current.set(selectedIdRef.current, el.scrollTop); setShowJump(el.scrollHeight - el.scrollTop - el.clientHeight > 320); }}>

...
import { bootstrapTailnetPeer, credentialStatus, deviceId, forgetDeviceCredential, importLegacyDeviceMetadata, isValidPairingCode, listDevices, normalizePairingCode, normalizeUrl, onBootstrapLog, openDevice, openTailscaleApproval, pairDevice, probe, removeDevice, saveDevice, scanTailnet } from "./lib";
...
function FleetApp() {
  ...
  const [port, setPort] = useState(30141);
  ...
  async function addPeer(peer: TailnetPeer) {
    const item: Device = {
      ...
      url: peer.url, source: "tailscale", favorite: false,
  ...
  return (
    <div className="app-shell">
        ...
        <section className="fleet-section">
          ...
          {filtered.length ? (
            <div className="device-grid">
              {filtered.map((device) => <DeviceCard key={device.id} device={device} status={statuses[device.id]} paired={paired[device.id]} onOpen={() => void openDevice(device)} onPair={() => setPairing(device)} onUnpair={() => setUnpairing(device)} onFavorite={() => void toggleFavorite(device)} onDelete={() => void deleteDevice(device.id)} onEdit={() => setEditing(device)} />)}
            ...
            <div className="empty"><Search size={28} /><h3>没有匹配的设备</h3><p>换个关键词试试。</p></div>
          ) : (
            <div className="fleet-empty-state">
              <div><Server size={20} /><span><strong>尚未保存设备</strong><small>通过上方发现设备，或手动添加 Tailnet 地址。</small></span></div>
              <button className="secondary-button compact" onClick={() => setModal("add")}><Plus size={15} />手动添加</button>
            </div>
          )}
        </section>
      </main>
      ...
      {modal === "discover" && <DiscoverModal scan={scan} scanning={scanning} setupOnly={scanMode === "setup"} devices={devices} port={port} onPort={setPort} onRescan={() => void startScan(scanMode === "discover")} onAdd={(peer) => void addPeer(peer)} onClose={() => setModal(null)} />}
...
function DeviceCard({ device, status, paired, onOpen, onPair, onUnpair, onFavorite, onDelete, onEdit }: { device: Device; status?: DeviceStatus; paired?: boolean; onOpen: () => void; onPair: () => void; onUnpair: () => void; onFavorite: () => void; onDelete: () => void; onEdit: () => void }) {
  ...
  return (
    <article className="device-card" style={{ "--accent": device.accent } as React.CSSProperties}>
      <div className="card-top">
        <div className="device-icon"><PiHubDeviceIcon os={device.os} size={28} /><span className={`state-indicator ${state}`} /></div>
      ...
      <div className="card-body"><h3>{device.name}</h3><div className="host"><span>{device.source === "tailscale" ? "TAILSCALE 网络" : "直连"}</span>{device.host}</div></div>
...
function DiscoverModal({ scan, scanning, setupOnly, devices, port, onPort, onRescan, onAdd, onClose }: { scan: TailnetScan | null; scanning: boolean; setupOnly: boolean; devices: Device[]; port: number; onPort: (port: number) => void; onRescan: () => void; onAdd: (peer: TailnetPeer) => void; onClose: () => void }) {
  const [bootstrapping, setBootstrapping] = useState(""); const [bootstrapMessage, setBootstrapMessage] = useState(""); const [approvalUrl, setApprovalUrl] = useState(""); const [sshPeer, setSshPeer] = useState<TailnetPeer | null>(null); const [sshUser, setSshUser] = useState("");
  const [logLines, setLogLines] = useState<Array<{ line: string; stream: string }>>([]);
  const logRef = useRef<HTMLPreElement>(null);
  useEffect(() => {
    let disposed = false; let unlisten: (() => void) | undefined;
    void onBootstrapLog((line, stream) => { if (!disposed) setLogLines((current) => [...current.slice(-600), { line, stream }]); })
  ...
  async function bootstrap(peer: TailnetPeer, username?: string) {
    ...
    try { const result = await bootstrapTailnetPeer(peer.dnsName || peer.ip, peer.os, username); setApprovalUrl(result.approvalUrl || ""); setBootstrapMessage(result.requiresApproval ? `${peer.name}：服务已安装，等待 Tailscale Serve 管理员授权。授权后重新扫描即可连接。` : result.output.includes("PIHUB_BOOTSTRAP_OK") ? `${peer.name}：${result.installed ? "PiHub Server 已安装" : "现有 PiHub Server 已验证"}，并已启用 Tailscale Serve。` : result.output); } catch (cause) { const text = cause instanceof Error ? cause.message : String(cause); const tail = text.split("\n").map((line) => line.trim()).filter(Boolean).pop() || "未知错误"; setBootstrapMessage(`${peer.name}：配置失败 — ${tail.slice(0, 140)}`); setLogLines((current) => [...current, { line: `✕ ${tail}`, stream: "stderr" }]); } finally { setBootstrapping(""); }
  }
  return <Modal wide title={setupOnly ? "SSH 一键安装" : "发现 Tailnet 设备"} subtitle={scanning ? "正在查找在线 Tailnet 设备…" : scan?.tailnet ? `已连接到 ${scan.tailnet}` : "通过本机 Tailscale 自动发现"} onClose={onClose}>
    ...
    {scanning ? <div className="scan-loading"><div className="radar"><Radar size={30} /><i /></div><h3>正在扫描你的 Tailnet</h3><p>{setupOnly ? "读取在线节点，不探测远端服务端口" : `检查在线设备的 HTTPS ${port} 端口`}</p></div> : !scan?.available ? <div className="scan-loading"><WifiOff size={30} /><h3>没有连接到 Tailscale</h3><p>{scan?.message || "请先安装并登录 Tailscale，然后重试。"}</p></div> : (
      <div className="peer-list">
        ...
        }) : <div className="scan-loading"><Server size={30} /><h3>Tailnet 中没有在线设备</h3></div>}{bootstrapMessage && <div className="bootstrap-message" role="status"><span>{bootstrapMessage}</span>{approvalUrl && <button onClick={() => void openTailscaleApproval(approvalUrl)}>打开 Tailscale 授权</button>}</div>}{sshPeer && <form className="windows-ssh-form" onSubmit={(event) => { event.preventDefault(); if (sshUser.trim()) void bootstrap(sshPeer, sshUser.trim()); }}><div><strong>配置 {sshPeer.name}</strong><span>{sshPeer.os?.toLowerCase() === "windows" ? "使用系统 OpenSSH key/agent，不会保存密码。" : "使用 Tailscale SSH 的普通用户，不允许以 root 安装。"}</span></div><input value={sshUser} onChange={(event) => setSshUser(event.target.value)} placeholder={sshPeer.os?.toLowerCase() === "windows" ? "Windows 用户名" : "Linux 用户名（例如 pi 或 ubuntu）"} autoFocus /><button type="button" onClick={() => setSshPeer(null)}>取消</button><button type="submit" disabled={!sshUser.trim()}>继续</button></form>}
      </div>
    )}

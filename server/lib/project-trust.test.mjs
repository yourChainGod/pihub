...
const {
  getProjectOperationTrustStatus,
  getProjectTrustStatus,
  projectTrustReloadOptions,
  trustProject,
  trustProjectOperation,
...
test("all Git repositories require operation trust without enabling project resources", async (t) => {
  const { cwd, agentDir } = await createProjectFixture(t);
  await execFileAsync("git", ["init", cwd]);

  assert.deepEqual(await getProjectOperationTrustStatus(cwd, agentDir), {
    requiresTrust: true,
    trusted: false,
  });
  assert.deepEqual(getProjectTrustStatus(cwd, agentDir), {
  ...
  assert.deepEqual(trustProjectOperation(cwd, agentDir), {
    requiresTrust: true,
    trusted: true,
  });
  assert.deepEqual(await getProjectOperationTrustStatus(cwd, agentDir), {
...
test("the trust API invalidates cached models and restricted runtimes", async () => {
  ...
  assert.match(source, /trustProjectOperation\(result\.cwd, agentDir\)/);
  assert.match(source, /getProjectOperationTrustStatus\(result\.cwd, agentDir\)/);
  assert.match(source, /getTrustedPihubRequestContext\(req\)/);
  assert.match(source, /getAllowedFileRoots\(\{ ownerId \}\)/);
  assert.match(source, /capabilities\.includes\("workspaces:read"\)/);
  assert.match(source, /capabilities\.includes\("workspaces:manage"\)/);

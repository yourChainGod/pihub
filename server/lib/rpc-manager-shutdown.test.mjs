...
test("readiness aborts immediately while extension binding is still pending", async (t) => {
  const inner = makePromptInner(() => Promise.resolve());
  inner.bindExtensions = () => new Promise(() => {});
  const wrapper = new AgentSessionWrapper(inner, {
  ...
  const readiness = wrapper.waitUntilReady(abort.signal);
  abort.abort();

  await assert.rejects(
    readiness,
    (error) => error?.name === "RpcSessionLifecycleError" && error.code === "START_ABORTED",
  );
});
...
test("shutdown closes admission synchronously and rejects every later command", async () => {
  let finishShutdown;
  const inner = {
    isBashRunning: false,
    extensionRunner: {
  ...
  assert.equal(wrapper.lifecycleState, "draining");
  await assert.rejects(
    wrapper.send({ type: "get_state" }),
    (error) => error?.name === "RpcSessionLifecycleError" && error.code === "DRAINING",
  );

  finishShutdown();
  await shuttingDown;
  assert.equal(wrapper.lifecycleState, "closed");
});

test("one shutdown deadline bounds hanging extension binding and hooks", async (t) => {
  const originalConsoleError = console.error;
  ...
  const inner = {
    isBashRunning: false,
    bindExtensions: () => never,
    extensionRunner: { emit: () => never },
    dispose() { disposed += 1; },
  };
  const wrapper = new AgentSessionWrapper(inner, {
    initialState: "starting",
  ...
  await assert.rejects(
    wrapper.shutdown(),
    (error) => error?.name === "RpcSessionLifecycleError" && error.code === "SHUTDOWN_TIMEOUT",
  );
  const elapsed = Date.now() - startedAt;

  assert.equal(disposed, 1);
  assert.equal(wrapper.lifecycleState, "closed");
  assert.ok(elapsed < 250, `shutdown exceeded its total deadline: ${elapsed}ms`);
});

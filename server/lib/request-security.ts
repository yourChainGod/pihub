...
function isTailscaleIpv6(hostname: string): boolean {
  return hostname.toLowerCase().replace(/^\[|\]$/g, "").startsWith("fd7a:115c:a1e0:");
}
...
export function isApiRequestHostAllowed(
  ...
  if (isLoopbackHostname(hostname) || hostname === "127.0.0.1" || hostname === "::1") return true;
  if (isTailscaleIpv4(hostname) || isTailscaleIpv6(hostname)) return true;

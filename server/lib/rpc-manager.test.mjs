...
test("RPC session startup preloads extension-registered providers before restoring models", async () => {
  ...
  assert.match(startupSource, /createSafeAgentSessionServices\(/);
  assert.doesNotMatch(startupSource, /\bcreateAgentSessionServices\(/);
...
test("normal session teardown paths use graceful extension shutdown", async () => {
  ...
  assert.match(deleteRouteSource, /deleteSessionTransaction\(\{/);
  assert.match(deleteRouteSource, /getRpcSession\(sessionId, ownerId\)\?\.shutdown\(\)/);
  assert.match(deleteRouteSource, /removeSessionOwner\(id, ownerId\)/);
  assert.match(trustRouteSource, /await destroyRpcSessionsForCwd\(result\.cwd\)/);
});

test("new RPC sessions commit ownership before registry visibility and roll it back on failure", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const bindIndex = startupSource.indexOf("await bindSessionOwner(realSessionId, ownerId)");
  const readyIndex = startupSource.indexOf("await wrapper.waitUntilReady(startupCancellation.signal)");
  const registryIndex = startupSource.indexOf("registry.set(realSessionId, wrapper)");

  assert.ok(bindIndex >= 0);
  assert.ok(readyIndex > bindIndex);
  assert.ok(registryIndex > readyIndex);
  assert.match(startupSource, /startupOwnershipId = realSessionId/);
  assert.match(startupSource, /await removeSessionOwner\(startupOwnershipId, ownerId\)/);
});

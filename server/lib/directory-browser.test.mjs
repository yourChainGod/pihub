...
test("fails closed when a directory exceeds the bounded entry limit", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "pi-web-browse-limit-"));
  try {
    await Promise.all(["a", "b", "c"].map((name) => mkdir(path.join(root, name))));
    const {
    ...
    assert.equal(MAX_BROWSE_DIRECTORY_ENTRIES, 10_000);
    await assert.rejects(
      listDirectories(root, 2),
      (error) => error instanceof DirectoryEntryLimitError,
    );
    await assert.rejects(listDirectories(root, MAX_BROWSE_DIRECTORY_ENTRIES + 1), {
      name: "TypeError",
    });
  } finally {
    await rm(root, { recursive: true, force: true });
